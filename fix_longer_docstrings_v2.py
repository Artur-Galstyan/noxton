...
content = content.replace(
    '"""Computes the sLSTM pointwise gates and updates the cell states (y, c, n, m)."""',
...
content = content.replace(
    '"""Executes a single recurrent step of the sLSTM cell, updating states and computing outputs."""',
...
content = content.replace(
    '"""Applies the sLSTM recurrent step across an entire sequence using a jax.lax.scan operation."""',
...
content = content.replace(
    '"""Computes the parallel stabilized matrix LSTM (mLSTM) mixing operation over a sequence."""',
...
content = content.replace(
    '"""Executes a single stabilized recurrent step for the matrix LSTM (mLSTM) formulation."""',
...
# Replace class level and method level using regex or split to avoid replacing wrong class methods.


def replace_in_class(cls_name, content, method_replacements):
...
sLSTMCell_replacements = {
    ...
    '"""Initializes and returns the empty initial states for the module."""': '"""Initializes and returns the empty states for the cell.\n        \n        Returns:\n            A tensor of initialized zero states for the sLSTM cell.\n        """',
}

sLSTMLayer_replacements = {
    '"""A complete sLSTM layer implementation, featuring input gating and an optional 1D causal convolution."""': '"""A complete sLSTM layer implementation.\n\n    This layer wraps the `sLSTMCell` with optional 1D causal convolution and computes\n    the necessary projections for the input, forget, output, and update gates. It serves\n    as a complete sub-layer component for sequential data modeling.\n    """',
...
xLSTMBlock_replacements = {
    ...
    '"""Initializes and returns the underlying states for the recurrent layers within the block."""': '"""Initializes and returns the underlying states for the block.\n        \n        Delegates the state initialization to the underlying xLSTM layer.\n        \n        Returns:\n            The initial states for the components within the block.\n        """',
}

content = replace_in_class("mLSTMCell", content, mLSTMCell_replacements)
content = replace_in_class("mLSTMLayer", content, mLSTMLayer_replacements)
content = replace_in_class("sLSTMCell", content, sLSTMCell_replacements)
content = replace_in_class("sLSTMLayer", content, sLSTMLayer_replacements)
content = replace_in_class("xLSTMBlock", content, xLSTMBlock_replacements)
