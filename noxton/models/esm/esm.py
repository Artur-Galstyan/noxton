...
import numpy as np
from beartype.typing import Any, Literal, Type, Union, cast
...
from .rotation import Rotation, RotationMatrix, RotationQuat
...
class ESMMultiHeadAttention(eqx.Module):
    ...
    def __call__(
        ...
        *,
        ...
        for layernorm_qkv_layer in self.layernorm_qkv:
            x = eqx.filter_vmap(layernorm_qkv_layer)(x)
        qkv = x
...
class UnifiedTransformerBlock(eqx.Module):
    ...
    def __call__(
        ...
        for ffn_layer in self.ffn:
            ffn_x = eqx.filter_vmap(ffn_layer)(ffn_x)
...
class ESMC(eqx.Module):
    ...
    def __call__(
        ...
        sequence_logits = x
        for sh_layer in self.sequence_head:
            sequence_logits = eqx.filter_vmap(sh_layer)(sequence_logits)
