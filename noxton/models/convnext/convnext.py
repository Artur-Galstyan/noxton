...
from noxton.utils import default_floating_dtype, dtype_to_str, get_cache_path
...
class ConvNeXt(StatefulLayer):
    ...
    def with_weights(
        ...
        """
        Loads a ConvNeXt variant with ImageNet-1K pretrained weights.
        If no key is provided, the default key (42) is used. Dtype defaults to float32.
        axis_name is unused (ConvNeXt uses LayerNorm, not BatchNorm).
        ...
        match model:
            case "convnext_tiny":
                convnext_model, state = _convnext_tiny(key, dtype=dtype)
                weights_key = "convnext_tiny_IMAGENET1K_V1"
            case "convnext_small":
                convnext_model, state = _convnext_small(key, dtype=dtype)
                weights_key = "convnext_small_IMAGENET1K_V1"
            case "convnext_base":
                convnext_model, state = _convnext_base(key, dtype=dtype)
                weights_key = "convnext_base_IMAGENET1K_V1"
            case "convnext_large":
                convnext_model, state = _convnext_large(key, dtype=dtype)
...
def _convnext_tiny(key, num_classes=1000, dtype: Any = None, inference: bool = False):
    if dtype is None:
        dtype = default_floating_dtype()
    assert dtype is not None
...
def _convnext_small(key, num_classes=1000, dtype: Any = None, inference: bool = False):
    if dtype is None:
        dtype = default_floating_dtype()
    assert dtype is not None
...
def _convnext_base(key, num_classes=1000, dtype: Any = None, inference: bool = False):
    if dtype is None:
        dtype = default_floating_dtype()
    assert dtype is not None
...
def _convnext_large(key, num_classes=1000, dtype: Any = None, inference: bool = False):
    if dtype is None:
        dtype = default_floating_dtype()
    assert dtype is not None
