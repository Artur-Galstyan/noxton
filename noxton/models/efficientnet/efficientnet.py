...
from statedict2pytree import autoconvert
...
from noxton.utils import default_floating_dtype, dtype_to_str
...
class MBConv(eqx.Module):
    ...
    has_expand: bool = eqx.field(static=True)
    block: list
    ...
    def __init__(
        ...
        self.has_expand = expanded_channels != cnf.input_channels
        if self.has_expand:
    ...
    def __call__(
        ...
        if self.has_expand:
            result, state = self.block[0](result, state)
            idx = 1

        result, state = self.block[idx](result, state)
        result = self.block[idx + 1](result, activation=jax.nn.silu)
        result, state = self.block[idx + 2](result, state)
...
class EfficientNet(eqx.Module):
    ...
    def with_weights(
        ...
        """
        Loads an EfficientNet variant with optional pretrained weights.
        If no key is provided, the default key (42) is used. Dtype defaults to float32.

        Returns:
            tuple[EfficientNet, eqx.nn.State]
        """
        return load_efficientnet(
...
def _with_weights(
    ...
    efficientnet, state = autoconvert((efficientnet, state), weights_dict, dtype=dtype)
...
def _efficientnet_b0(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
...
def _efficientnet_b1(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_b2(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_b3(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_b4(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_b5(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_b6(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_b7(key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"):
    inverted_residual_setting, last_channel = _efficientnet_conf(
...
def _efficientnet_v2_s(
    key, n_classes=1000, dtype: Any = None, axis_name: str = "batch"
):
    ...
    *,
    ...
    match model:
        case "efficientnet_b0":
            efficientnet, state = _efficientnet_b0(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b1":
            efficientnet, state = _efficientnet_b1(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b2":
            efficientnet, state = _efficientnet_b2(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b3":
            efficientnet, state = _efficientnet_b3(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b4":
            efficientnet, state = _efficientnet_b4(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b5":
            efficientnet, state = _efficientnet_b5(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b6":
            efficientnet, state = _efficientnet_b6(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_b7":
            efficientnet, state = _efficientnet_b7(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_v2_s":
            efficientnet, state = _efficientnet_v2_s(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_v2_m":
            efficientnet, state = _efficientnet_v2_m(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case "efficientnet_v2_l":
            efficientnet, state = _efficientnet_v2_l(
                key, n_classes, dtype=dtype, axis_name=axis_name
            )
        case _:
