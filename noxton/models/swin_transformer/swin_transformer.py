...
from noxton.functions import shifted_window_attention
...
class SwinTransformer(eqx.Module):
    ...
    def from_pretrained(
        ...
        """Load a SwinTransformer model, optionally with pretrained ImageNet weights.

        Args:
            model: Architecture variant to load.
            weights: Pretrained weight set to load. Pass ``None`` for random
                initialization.
            key: JAX PRNG key used for random initialization. Defaults to
                ``jax.random.key(42)`` when ``None``.
            dtype: Parameter dtype. Defaults to the global floating dtype.
            axis_name: Unused — SwinTransformer has no BatchNorm. Included for
        ...
        match model:
            case "swin_t":
                result = _swin_t(key, False, dtype=dtype)
            case "swin_s":
                result = _swin_s(key, False, dtype=dtype)
            case "swin_b":
                result = _swin_b(key, False, dtype=dtype)
            case "swin_v2_t":
                result = _swin_v2_t(key, False, dtype=dtype)
            case "swin_v2_s":
                result = _swin_v2_s(key, False, dtype=dtype)
            case "swin_v2_b":
                result = _swin_v2_b(key, False, dtype=dtype)
            case _:
                raise ValueError(f"Unknown SwinTransformer model: {model}")

        if weights:
            _assert_model_and_weights_fit_swin(model, weights)
            result = _swin_with_weights(result, weights, True, dtype=dtype)
