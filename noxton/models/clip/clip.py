...
import noxton.functions as F
from noxton.nn import BatchNorm, MultiheadAttention
...
class ResidualAttentionBlock(eqx.Module):
    ...
    def __init__(
        ...
        self.mlp = MLP(
            c_fc=eqx.nn.Linear(d_model, d_model * 4, key=subkeys[1], dtype=dtype),
            c_proj=eqx.nn.Linear(d_model * 4, d_model, key=subkeys[2], dtype=dtype),
        )
    ...
    def __call__(self, x: Array, attn_mask: Array | None = None):
        ...
        c_proj_out = self.mlp(ln_2_output)
...
class CLIP(eqx.Module):
    ...
    def from_pretrained(
        ...
        """
        Loads a CLIP variant with pretrained weights.
