...
def selective_scan(
    ...
    """Compute the Mamba selective scan (chunked parallel associative scan).

    Implements the core SSM recurrence from *Mamba: Linear-Time Sequence
    Modeling with Selective State Spaces* (Gu & Dao, 2023) using a two-level
    chunked scan strategy for hardware efficiency:

    1. **Intra-chunk scan** – within each chunk of ``chunk_size`` steps, run a
       sequential scan starting from a zero hidden state.
    2. **Inter-chunk correction** – propagate boundary hidden states across
       chunk boundaries and add the resulting correction term to the outputs.
    ...
    Args:
        u: Input sequence of shape ``(seq_length, d_inner)``.
        delta: Time-step projection of shape ``(seq_length, d_inner)``.
        A: State transition matrix of shape ``(d_inner, d_state)``.
        B: Input-dependent B matrices of shape
            ``(seq_length, d_inner, d_state)``.
        C: Input-dependent C matrices of shape
            ``(seq_length, d_inner, d_state)``.
        D: Skip-connection scale of shape ``(d_inner,)``.
        chunk_size: Number of time steps per chunk for the intra-chunk scan.
            Larger values use more memory but may be faster on modern
            accelerators. Defaults to ``128``.

    Returns:
        Output array of shape ``(seq_length, d_inner)`` containing the SSM
        outputs ``y_t`` for every time step.
    ...
    Example:
        ...
        >>> delta = jax.nn.softplus(jax.random.normal(key, (L, d)))
        >>> A = -jnp.ones((d, N))
        >>> B = jax.random.normal(key, (L, d, N))
        >>> C = jax.random.normal(key, (L, d, N))
        >>> D = jnp.ones(d)
